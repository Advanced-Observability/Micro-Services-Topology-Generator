...
const (
	DEFAULT_CONFIG_PATH     = "../config.yaml"                                                 // Default path to config file
	VERSION                 = "0.0.11"                                                         // Version of the service
...
func readConfig(file string) (map[string]Service, error) {
	...
	for name, service := range data {
		...
		// create the _Endpoints and _MaxRespsize field, indexed by the entrypoint string
		if len(service.Endpoints) > 0 {
			...
			for _, endp := range service.Endpoints {
				...
				if endp.Respsize == 0 {
					endp.Respsize = DEFAULT_PACKET_SIZE
				}

				// need to be MIN_PACKET_SIZE bytes minimum
				endp.Respsize = max(endp.Respsize, MIN_PACKET_SIZE)

				// compute maximum packet size in all endpoints
				service._MaxRespsize = max(service._MaxRespsize, endp.Respsize)
