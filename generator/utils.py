...
def check_arguments(args) -> str:
    ...
    parser.add_argument(
        "--ip", required=True, choices=[4, 6], type=int, help="IP version"
    )
    parser.add_argument(
        "--https", action="store_true", help="Use HTTPS for communication"
    )
    ...
    if args.kubernetes:
        ...
        print_info(
            f"\t- IP range for services: {kubernetes.Kubernetes.get_service_ip_range()}"
        )
        print_info(f"\t- IP range for pods: {kubernetes.Kubernetes.get_pod_ip_range()}")
        print_info(
            f"\t- Number of nodes in cluster: {kubernetes.Kubernetes.get_nb_nodes()}"
        )
...
def check_ovs_kernel_module() -> bool:
    ...
    res = subprocess.run(
        constants.OVS_CHECK_CMD, shell=True, stdout=subprocess.PIPE, check=False
    )
