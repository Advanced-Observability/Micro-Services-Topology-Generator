...
A configuration file is a `yaml` file describing all the entities of the architecture along with the network links between them.
...
- `<respsize>` is the size of the responses (in bytes) returned by the service for the endpoint;
...
If the microservice does not contact any other service, the `connections` field can be omitted.
...
- `neighbors` represents the entity (can be service, router, switch, firewall or external) to which the router is connected.

The specified `neighbors` need to be coherent with the `connections` of the other services/externals.
For instance, if a service specified `r1->r2->db`, the router `r1` needs to specify `r2` as neighbor while router `r2` needs to specify `db`.
...
The values in the template are the following:
- `docker_image` is the name of the Docker image, which must be available on the host;
- `port` is a port to be opened for the container. The container can have multiple open ports;
- `path` is a connection as specified for a service. The container can have multiple connections.
...
- `id` is the **optional** VLAN id.

You can also omit the `vlan` field by using the following template:
``` yaml
<switch_name>:
  type: switch
  neighbors:
...
> [!CAUTION]
> As of v0.0.7, switches **only** works using Docker Compose export. Kubernetes export is **not** yet supported.
...
See [6_modify_traffic.yaml](./configuration_examples/6_modify_traffic.yaml) for an example using network options.
...
See [7_timers.yaml](./configuration_examples/7_timers.yaml) for an example using these timers.
...
More examples are available in the folder [`configuration_examples`](../config-examples/).
