...
// See https://github.com/Advanced-Observability/cross-layer-telemetry/tree/main/library
...
func clt_enable(socketFD uint32, traceIDHigh uint64, traceIDLow uint64, spanID uint64) error {
	...
	if err != nil {
		log.Println("Error while establishing generic netlink socket")
		return err
	}
	defer c.Close()
	...
	if err != nil {
		log.Println("Cannot create attributes for generic netlink message")
		return err
	}
	...
	if _, err = c.Execute(req, family.ID, flags); err != nil {
		log.Println("clt_enable execute error: " + err.Error())
		return err
	}
...
func clt_disable(socketFD uint32) error {
	...
	flags := netlink.Request | netlink.Acknowledge
	if _, err = c.Execute(req, family.ID, flags); err != nil {
		log.Println("clt_disable execute error: " + err.Error())
		return err
	}
